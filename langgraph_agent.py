...
def _openclaw_node(state: DigestState) -> DigestState:
    config = state.get("config", {})
    trace = state.get("trace", {})
    attempt = int(state.get("attempt", 1))
    try:
        top, focus, asof, errors = openclaw_stage(
            config=config,
            trace=trace,
...
def _after_collect(state: DigestState) -> str:
    return _retry_or_next(state, "openclaw")
...
def _after_openclaw(state: DigestState) -> str:
...
def build_workflow():
    ...
    graph.add_node("openclaw", _openclaw_node)
    ...
    graph.add_conditional_edges("collect", _after_collect, {"collect": "collect", "openclaw": "openclaw", "persist": "persist"})
    graph.add_conditional_edges("openclaw", _after_openclaw, {"collect": "collect", "enrich": "enrich", "persist": "persist"})
