...
def extract_intent_keywords(intent_text: str) -> list[str]:
    ...
    if not text:
        return []
    raw_tokens = re.findall(r"[a-z0-9\-\+\.]{2,}|[\u4e00-\u9fff]{2,}", text)
    stopwords = {"新闻", "资讯", "相关", "看看", "查询", "我想", "一下", "最近", "关于", "生成", "ai"}
    out: list[str] = []
    ...
    *,
    ...
    user_prompt = {
        ...
        "rules": {
            ...
            "per_item": "每条生成title_cn、core_cn、value_cn。全部必须为中文。title_cn是中文新闻标题且不能为空；若原文标题为英文需翻译成中文，不要保留英文。core_cn为核心内容：若原文为英文则翻译成中文，若原文已是中文则按原意完整表述关键信息，不做压缩式精简；value_cn为该动态对业务/产品/研发流程的应用价值（不超过80字）。禁止输出泛化空话。",
